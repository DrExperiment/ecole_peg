...
useEffect(() => {
  ...
  useEffect(() => {
    ...
    axios.get(`http://localhost:8000/api/factures/eleve/${resolvedParams.id}/`)
      .then((res) => {
        const nonPayees = res.data.filter((f: Facture) => f.montant_restant > 0);
        setFactures(nonPayees);
        setFactureId("");
        setMontant("");
      })
      .catch(() => {
