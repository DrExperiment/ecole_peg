...
export default function NouvelleSessionPage() {
  ...
  const onSoumission = useCallback(
    async (donnees: object) => {
      const donneesCompletes = {
        ...donnees,
        date_debut: date_debut ? format(date_debut, "dd-MM-yyyy") : undefined,
        date_fin: date_fin ? format(date_fin, "dd-MM-yyyy") : undefined,
        id_cours,
        id_enseignant,
      };

      try {
        await fetchApi("/cours/session/", {
          method: "POST",
  ...
  useEffect(() => {
    async function fetchCours() {
      try {
        const donnees: Cours[] = await fetchApi("/cours/cours/");
    ...
    async function fetchEnseignants() {
      try {
        const donnees: Enseignant[] = await fetchApi("/cours/enseignants/");
