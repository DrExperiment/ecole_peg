    ...
    def clean(self):
        ...
        if self.inscription:
            if self.cours_prive or self.eleve:
                raise ValidationError("Si 'inscription' est défini, 'cours_prive' et 'eleve' doivent être null.")
        
        if self.cours_prive:
            if not self.eleve:
                raise ValidationError("Si 'cours_prive' est défini, 'eleve' doit être défini.")
            if self.inscription:
                raise ValidationError("Si 'cours_prive' est défini, 'inscription' doit être null.")
        
        if self.eleve:
            if not self.cours_prive:
                raise ValidationError("Si 'eleve' est défini, 'cours_prive' doit être défini.")
            if self.inscription:
                raise ValidationError("Si 'eleve' est défini, 'inscription' doit être null.")
...
class Paiement(models.Model):
    ...
    methode_paiement = models.CharField(max_length=3, choices=MethodePaiementChoices.choices)
