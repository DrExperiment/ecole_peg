...
class Personne(models.Model):
    ...
    numero = models.CharField(
        ...
        blank=True,
        null=True,
        validators=[RegexValidator(r'^\d+$', message="Le numéro doit contenir uniquement des chiffres.")]
    )
    npa = models.CharField(
        max_length=4,
        blank=True,
        null=True,
        validators=[RegexValidator(r'^\d{4}$', message="Le NPA doit contenir exactement 4 chiffres.")]
    )
    ...
    def clean(self):
        ...
        if any([self.rue, self.numero, self.npa, self.localite]) and not all([self.rue, self.numero, self.npa, self.localite]):
            raise ValidationError("Tous les champs d'adresse doivent être remplis ou laissés vides.")
...
class Eleve(Personne):
    ...
    garant = models.ForeignKey(Garant, on_delete=models.SET_NULL, null=True, blank=True, related_name="eleves")
...
class Document(models.Model):
    ...
    fichier = models.FileField(
        ...
        validators=[FileExtensionValidator(allowed_extensions=['pdf', 'jpg', 'jpeg', 'png']),file_size_validator]
    )
